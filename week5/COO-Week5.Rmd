...
``` {r}
baits <- c("A", "A", "A", "A", 'B', 'B', 'F', 'G')
preys <- c("B", "C", "D", "E", 'F', 'G', 'I', 'I')
...
SuppS6 <- read_excel('C:/Users/annev/OneDrive/Bureaublad/Utrecht/Omics in oncology/week5coding/week5/science.abf3066_Table_S6.xlsx', sheet = 2)
...
interactionGraph_plot <- layoutGraph(interactionGraph, layoutType="neato")

renderGraph(interactionGraph_plot)
...
```{r}
baitssub <- SuppS6_sub$Bait
preyssub <- SuppS6_sub$Prey
...
```{r}
interactionGraphsub <- makeproteingraph(baitssub, preyssub)
interactionGraphsub_plot <- layoutGraph(interactionGraphsub, layoutType="neato")

renderGraph(interactionGraphsub_plot)
...
color_gradient <- colorRampPalette(c("lightgrey", "pink"))

# Map DIS values to colors
edge_colors <- color_gradient(100)[as.numeric(cut(SuppS6_sub$DIS, breaks=100))]

# Assign colors to edges
edgeRenderInfo(ppiGraph) <- list(col=edge_colors)

# Define bait nodes
shape <- rep("rectangle", length(SuppS6_sub$Bait))  # Rectangles for baits
names(shape) <- SuppS6_sub$Bait

baitFont <- rep(48, length(SuppS6_sub$Bait))  # Large font for baits
names(baitFont) <- SuppS6_sub$Bait

fill <- rep("lightblue", length(SuppS6_sub$Bait))  # Default blue for baits
names(fill) <- SuppS6_sub$Bait

lwd <- rep(2, length(SuppS6_sub$Bait))  # Thick border for baits
names(lwd) <- SuppS6_sub$Bait

# Formatting prey nodes (Unlabeled, Small Circles)
prey_nodes <- SuppS6_sub$Prey
preyFont <- rep(1, length(prey_nodes))  # Small font for prey labels
names(preyFont) <- prey_nodes

preyShape <- rep("circle", length(prey_nodes))  # Small circles for preys
names(preyShape) <- prey_nodes

preySize <- rep(10, length(prey_nodes))  # Make them smaller
names(preySize) <- prey_nodes

# Combine bait and prey nodes
all_nodes <- c(SuppS6_sub$Bait, prey_nodes)

# Create a combined font vector
font <- c(baitFont, preyFont)
names(font) <- all_nodes

# Create a combined shape vector
shape <- c(shape, preyShape)
names(shape) <- all_nodes

# Create a combined fill vector
fill <- c(fill, rep("white", length(prey_nodes)))  # Default fill for prey nodes
names(fill) <- all_nodes

# Create a combined lwd vector
lwd <- c(lwd, rep(1, length(prey_nodes)))  # Default border width for prey nodes
names(lwd) <- all_nodes

# Apply node formatting
nodeRenderInfo(ppiGraph) <- list(fontsize=font, fill=fill, shape=shape, lwd=lwd, lty=1)
...
```{r}
node_degrees <- degree(interactionGraphsub)
...
None with a degree of 0, most have a degree of 1, two have a degree over 40.
...
```{r}
connected_comp <- connectedComp(interactionGraphsub_plot)
